...
namespace vorpinventory_cl
{
    public class Utils : BaseScript
    {
        ...
        public static void cleanAmmo(int id)
        {
            ...
            {
                API.SetPedAmmo(API.PlayerPedId(), (uint)API.GetHashKey(vorp_inventoryClient.userWeapons[id].getName()),
                               0);
                foreach (var ammo in vorp_inventoryClient.userWeapons[id].getAllAmmo())
                {
        ...
        public static void useWeapon(int id)
        {
            ...
            {
                API.GiveWeaponToPed_2(API.PlayerPedId(),
                                      (uint)API.GetHashKey(vorp_inventoryClient.userWeapons[id].getName()), 0, true,
                                      true, 3, false, 0.5f, 1.0f, 752097756, false, 0, false);
                Function.Call((Hash)0xADF692B254977C0C, API.PlayerPedId(),
                              API.GetHashKey(vorp_inventoryClient.userWeapons[id].getName()), 0, 0, 0, 0);
                API.SetPedAmmo(API.PlayerPedId(), (uint)API.GetHashKey(vorp_inventoryClient.userWeapons[id].getName()),
                               0);
                foreach (var ammos in vorp_inventoryClient.userWeapons[id].getAllAmmo())
                {
        ...
        public static void oldUseWeapon(int id)
        {
            ...
            API.GiveWeaponToPed_2(API.PlayerPedId(),
                                  (uint)API.GetHashKey(vorp_inventoryClient.userWeapons[id].getName()), 0, true, true,
                                  2, false, 0.5f, 1.0f, 752097756, false, 0, false);
            Function.Call((Hash)0xADF692B254977C0C, API.PlayerPedId(),
                          API.GetHashKey(vorp_inventoryClient.userWeapons[id].getName()), 0, 1, 0, 0);
            ...
            TriggerServerEvent("vorpinventory:setUsedWeapon", id, vorp_inventoryClient.userWeapons[id].getUsed(),
                               vorp_inventoryClient.userWeapons[id].getUsed2());
        }
        ...
        public static void addItems(string name, int cuantity)
        {
            ...
            {
                vorp_inventoryClient.useritems.Add(name, new ItemClass(cuantity,
                                                                       vorp_inventoryClient.citems[name]["limit"],
                                                                       vorp_inventoryClient.citems[name]["label"], name,
                                                                       "item_standard", true,
                                                                       vorp_inventoryClient
                                                                               .citems[name]["can_remove"]));
            }
        }
        ...
        public static async Task DrawText3D(Vector3 position, string text)
        {
            ...
            var factor = text.Length / 150.0F;
            Function.Call((Hash)0xC9884ECADE94CB34, "generic_textures", "hud_menu_4a", _x, _y + 0.0125F,
                          0.015F + factor,
        ...
        public static List<int> getNearestPlayers()
        {
            ...
            var localPed = API.PlayerPedId();
            var coords = API.GetEntityCoords(localPed, true, true);
            var closestPlayers = new List<int>();
            ...
            {
                var target = API.GetPlayerPed(player);
                if (target != localPed)
                {
                    var targetCoords = API.GetEntityCoords(target, true, true);
                    var distance = API.GetDistanceBetweenCoords(targetCoords.X, targetCoords.Y, targetCoords.Z,
